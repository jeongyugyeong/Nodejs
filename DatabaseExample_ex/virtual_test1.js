...
function connectDB() {
	...
	database.on('open', function () {
		console.log('데이터베이스에 연결되었습니다. : ' + databaseUrl); //a1
...
function createUserSchema() {
	...
	UserSchema
	  ...
	  .set(function(info) {
	    ...
	    console.log('virtual info 설정함 : %s, %s', this.id, this.name);//a4
	  })
...
function doTest() {
	...
	user.save(function(err) {
	    ...
	    console.log("사용자 데이터 추가함.");//a7

	    findAll();
	});
	
	console.log('info 속성에 값 할당함.');//a5
	console.log('id : %s, name : %s', user.id, user.name);//a6
}
